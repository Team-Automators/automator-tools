...
const TTL_MS = 30 * 24 * 60 * 60 * 1000; // 30 days (client also silently re-auths on expiry)

function secret() {
  // Accepts SESSION_SECRET or SESSIONS_SECRET (either spelling), then falls back
  // to the OAuth client secret only as a last resort.
  const s = process.env.SESSION_SECRET || process.env.SESSIONS_SECRET || process.env.GHL_CLIENT_SECRET;
  if (s) return s;
  // Dev fallback — never rely on this in production.
  console.warn('[session] No SESSION_SECRET/SESSIONS_SECRET set — using insecure dev secret.');
