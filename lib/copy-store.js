...
async function createCustomer(locationId, { name, email = '', ownerUserId = '' }) {
  const customers = await getCustomers(locationId);
  const customer  = { id: genId(), ownerUserId, name, email, createdAt: Date.now() };
...
// One-time migration: assign all unowned (legacy) customer folders to a user.
async function claimLegacyCustomers(locationId, userId) {
  if (!userId) return 0;
  const customers = await getCustomers(locationId);
  let count = 0;
  for (const c of customers) if (!c.ownerUserId) { c.ownerUserId = userId; count++; }
  if (count) {
    if (redis) await redis.set(`cust:${locationId}`, customers);
    else mem.customers[locationId] = customers;
  }
...
async function getAllCopies(locationId) {
  ...
  if (redis) {
    // One MGET instead of a GET per copy (was an N+1 that made big backups slow).
    const vals = await redis.mget(...idx.map(r => `copy:${r.id}`));
...
async function importData(locationId, userId, { customers = [], copies = [] } = {}) {
  ...
  for (const c of customers) if (c?.id && !custIds.has(c.id)) { cust.push({ ...c, ownerUserId: c.ownerUserId || userId || '' }); custIds.add(c.id); }
  ...
  let added = 0;
  // Batch the per-copy writes into one pipeline instead of a round-trip each.
  const pipe = redis ? redis.pipeline() : null;
  for (const raw of copies) {
    if (!raw?.id) continue;
    const c = { ...raw, locationId, ownerUserId: raw.ownerUserId || userId || '' };
    if (pipe) pipe.set(`copy:${c.id}`, c); else mem.copies[c.id] = c;
...
module.exports = { STATUSES, genId, getCustomers, createCustomer, updateCustomer, deleteCustomer, getCopyIndex, getCopy, getAllCopies, saveCopy, updateCopy, updateStatus, deleteCopy, getCustomerCopies, claimLegacy, claimLegacyCustomers, importData };
