...
async function create(locationId, { title, customerId, customerName, stage, service, dueDate, waitingOn, clickupTaskId, clickupTaskName, ownerUserId }) {
  ...
  const task = {
    ...
    stage:           stage || 'urgent',
    service:         service         || '',   // pipeline column (setup-calls, funnels, …)
    dueDate:         dueDate         || '',   // 'YYYY-MM-DD'
    waitingOn:       waitingOn       || '',   // '' | 'client' | 'consultant'
...
async function update(locationId, taskId, fields) {
  ...
  const allowed = ['title', 'stage', 'customerId', 'customerName', 'service', 'dueDate', 'waitingOn', 'clickupTaskId', 'clickupTaskName'];
...
// One-time migration: assign all unowned (legacy) tasks to a user.
async function claimLegacy(locationId, userId) {
  if (!userId) return 0;
  const tasks = await getAll(locationId);
  let count = 0;
  for (const t of tasks) if (!t.ownerUserId) { t.ownerUserId = userId; count++; }
  if (count) await saveAll(locationId, tasks);
  return count;
}

// Detach a deleted customer from tasks (keep the task, clear the link).
async function detachCustomer(locationId, customerId) {
  const tasks = await getAll(locationId);
  let changed = false;
  for (const t of tasks) {
    if (t.customerId === customerId) { t.customerId = ''; t.customerName = ''; changed = true; }
  }
  if (changed) await saveAll(locationId, tasks);
  return changed;
}

// Merge tasks from a backup (upsert by id; never deletes).
async function importData(locationId, userId, tasks = []) {
  const cur = await getAll(locationId);
  const byId = new Map(cur.map(t => [t.id, t]));
  for (const raw of tasks) {
    if (!raw?.id) continue;
    const t = { ...raw, ownerUserId: raw.ownerUserId || userId || '', notes: normalizeNotes(raw.notes) };
    if (byId.has(t.id)) Object.assign(byId.get(t.id), t);
    else { cur.push(t); byId.set(t.id, t); }
  }
  await saveAll(locationId, cur);
  return tasks.length;
}

module.exports = { getAll, create, update, addNote, patchNote, deleteNote, remove, renameCustomer, detachCustomer, claimLegacy, importData };
