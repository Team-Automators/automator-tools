...
const archMem       = require('./architect-memory-store');

const FORMAT  = 'automator-backup';
const VERSION = 1;

async function exportAll(locationId, userId, { includeSettings = true } = {}) {
  const [customers, allCopies, tasks, pipeline, hooks, brandVoice, feedback, settings, architectMemory] = await Promise.all([
    ...
    archMem.getMemory(locationId, userId).catch(() => null),
  ]);

  const mine = (arr) => (Array.isArray(arr) ? arr : []).filter(x => (x.ownerUserId || '') === (userId || ''));
  ...
  return {
    ...
    userId,
    data: {
      customers:  mine(customers),  // per-user folders
      ...
      architectMemory: architectMemory && (architectMemory.playbook || (architectMemory.examples || []).length) ? architectMemory : null,
    },
  };
}
...
async function importAll(locationId, userId, payload) {
  ...
  if (d.architectMemory) await archMem.restore(locationId, userId, d.architectMemory);
  ...
  return {
    ...
    counts: {
      ...
      architectMemory: d.architectMemory ? 1 : 0,
    },
    result,
  };
}
