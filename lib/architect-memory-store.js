...
async function reset(loc, uid) {
  const empty = { playbook: '', examples: [], count: 0, updatedAt: Date.now() };
  await write(loc, uid, empty);
  return empty;
}

// Restore a whole memory record (backup import). Merges by keeping the richer
// side: longer playbook, higher count, unioned examples.
async function restore(loc, uid, rec) {
  if (!rec) return;
  const cur = await getMemory(loc, uid);
  const merged = {
    playbook: (rec.playbook || '').length >= (cur.playbook || '').length ? (rec.playbook || '') : cur.playbook,
    examples: [...(rec.examples || []), ...(cur.examples || [])].slice(0, 25),
    count: Math.max(cur.count || 0, rec.count || 0),
    updatedAt: Date.now(),
  };
  await write(loc, uid, merged);
  return merged;
}

module.exports = { getMemory, recordBuild, setPlaybook, reset, restore };
