...
// Merge hooks from a backup (upsert by id; never deletes). Rebuilds the token
// index so incoming-webhook routing keeps working after a restore.
async function importData(locationId, userId, hooks = []) {
  const cur = await getAll(locationId);
  const byId = new Map(cur.map(h => [h.id, h]));
  for (const raw of hooks) {
    if (!raw?.id) continue;
    const h = { ...raw, ownerUserId: raw.ownerUserId || userId || '' };
    if (byId.has(h.id)) Object.assign(byId.get(h.id), h);
    else { cur.push(h); byId.set(h.id, h); }
    if (h.incomingToken) await writeTokenIndex(h.incomingToken, locationId, h.id);
  }
  await saveAll(locationId, cur);
  return hooks.length;
}

module.exports = { getAll, saveAll, writeTokenIndex, create, update, remove, renameCustomer, detachCustomer, claimLegacy, findByCustomer, findByToken, recordIncoming, importData };
