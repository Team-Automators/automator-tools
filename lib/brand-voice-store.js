...
// Overwrite the whole feedback list (used by backup restore).
async function setFeedback(locationId, userId, list) {
  const arr = Array.isArray(list) ? list : [];
  if (redis) await redis.set(fKey(locationId, userId), arr, { ex: TTL });
  else mem.feedback[fKey(locationId, userId)] = arr;
}

module.exports = { getVoice, setVoice, clearVoice, getFeedback, addFeedback, clearFeedback, setFeedback, getSession, setSession };
