...
async function resolve(locationId) {
  ...
  for (const inst of installs) {
    ...
    // GHL access tokens expire ~daily; the refresh token is long-lived.
    // Resolve a *fresh* agency token (getAccessToken refreshes + re-persists
    // when expired) instead of the possibly-stale one from findAll(), or minting
    // would 401 a day after install and every location would fall to PIT.
    const agencyToken =
      (await oauthStore.getAccessToken(inst.locationId).catch(() => null)) || inst.access_token;
    if (!agencyToken) continue;
    try {
      const token = await mintLocationToken(agencyToken, inst.companyId, locationId);
      if (token) {
