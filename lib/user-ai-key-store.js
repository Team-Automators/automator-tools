...
async function set(email, { provider, apiKey, model, shared = false, sharedBy = '' }) {
  if (!norm(email)) return;
  const rec = { provider: provider || '', apiKey: apiKey || '', model: model || '', shared: !!shared, sharedBy: sharedBy || '', updatedAt: Date.now() };
