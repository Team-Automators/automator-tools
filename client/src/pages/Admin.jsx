...
const isActive = (ts) => ts && (Date.now() - ts) < 15 * 60 * 1000  // seen in last 15 min

export default function Admin() {
  const claims = getSessionClaims()
  const [users, setUsers] = useState([])
  const [loading, setLoading] = useState(true)
  const [busy, setBusy] = useState('')      // email currently acting on
  ...
  if (!claims?.adm) {
    return (
      <>
        <div className="topnav"><div className="topnav-left"><span className="breadcrumb-current">Admin</span></div></div>
        <div className="content">
          <div className="card empty-state" style={{ padding: 40 }}>
            <div className="empty-title">Admins only</div>
            <div className="empty-sub">Your account doesn’t have admin access to this console.</div>
          </div>
        </div>
      </>
    )
  ...
  return (
    <>
      <div className="topnav">
        <div className="topnav-left"><span className="breadcrumb-current">Admin</span></div>
      ...
      <div className="content">
        <div className="page-title" style={{ marginBottom: 12 }}>User Administration</div>
        ...
        <div style={{ fontSize: '.78rem', color: 'var(--sub)', marginBottom: 18 }}>
          Everyone who has signed in. Revoke a user’s AI key or force them to sign out (which also blocks re-login until you restore it).
        </div>
