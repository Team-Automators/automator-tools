...
// or offer stack). Identify it by its registration-page markers.
...
export default function CopywritersChat() {
  ...
  useEffect(() => {
    if (autoCycle && mockup && !mockup.loading && mockup.html && !mockup.error) {
      const DELAY = 20 // seconds between designs
  ...
  // Live elapsed timer while a preview is generating
  useEffect(() => {
                ...
                {!mockup?.loading && mockup?.html && (
                  <>
                    ...
                    <button
                      ...
                      onClick={() => {
                        api.mockupFeedback(mockup.style, 'skip') // manual regenerate = this one wasn't kept
                        setAutoCycle(false)
                        clearTimeout(cycleTimerRef.current)
                        clearInterval(countdownRef.current)
